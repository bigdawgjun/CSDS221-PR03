...
const ComputationBox = ({handleAnswer, answeredCorrectly, winner, draw, overallWin}) => {
  const [num1, setNum1] = useState(Math.floor(Math.random() * 90) + 10);
  const [num2, setNum2] = useState(Math.floor(Math.random() * 90) + 10);
  const [operator, setOperator] = useState(['+', '-', '*'][Math.floor(Math.random() * 3)]);
  ...
  return (
    <div className='quizbox'>
      <div>
        ...
        <span>{num2}</span>
        <span>=</span>
        <input type="number" value={userAnswer} onChange={handleAnswerChange} />  
        <button style={{"backgroundColor": "aqua" ,}} onClick={checkAnswer} disabled={countdown === 0}>Check Answer</button>
        </div>
        }
      </div>
      <div>{message}</div>
